...
liste_proprietes_iterations = cell(length(kappa),nb_iter_LDC_max+1); % length(devFactor)
sTime = zeros(1,length(kappa));
amplitude_bruit_Gaussien_U = 0 % 0:0.05:0.5;

% for j_param = 1:2
    for aux = 1:length(kappa) % i_param
        ...
        lambda_r0 = 41382.; % [Pa]
        lambda_i0 = 0.; % [Pa]
        mu_r0 = 1.5*1743. % [Pa] % devFactor(i_param*(j_param-1)+i_flag*(2-j_param))*1743.;
        mu_i0 = 0.75*174.3; % [Pa] % devFactor(i_param*(2-j_param)+i_flag*(j_param-1))*174.3;
        rho_0 = 1020.; % [kg/m^3]
        ...
        %kappa = 0.01*1e13; % convergence extremement lente 
        % kappa = 1*1e13 ; % bonne convergence si pas de bruit sur donnees
        %kappa = 100*1e13
        ...
        liste_proprietes_iterations{aux,n_iter_LDC} = struct_param_comportement_a_identifier.mat_param(struct_param_comportement_a_identifier.vec_numeros_parametres_a_identifier,:);
        ...
        % Debut du critere sur la convergence (utile pour id)

        while ( (~test_convergence_LDC) && ( n_iter_LDC <= nb_iter_LDC_max) ) 
        ...
        %            [Ks_global,Fs_global,U_impose_global,vec_n_DDL_conserves_global,vec_n_DDL_supprimes_global] = maj_matrices(K_global,vec_F_global,liste_DL_bloque);
                ...
                elseif ( strcmp(type_identification,'MERC') == 1 )
                    ...
                    disp(['                valeur de kappa : ' num2str(kappa(aux))]);
                    ...
                    vec_s_global = [vec_s_T.' , vec_s_K_tilde.'                  , conj(vec_s_K_tilde).'            , (-kappa(aux)*vec_s_D.')];
                    K_global = sparse(vec_i_global,vec_j_global,vec_s_global,2*size(K_tilde,1),2*size(K_tilde,2));
                    clear vec_i_K_tilde vec_j_K_tilde vec_s_K_tilde;
                    clear vec_i_T vec_j_T vec_s_T;
                    clear vec_i_D vec_j_D vec_s_D;
        ...
        %             clear K M T K_tilde D;
                    vec_F_global = zeros(size(K_global,1),1);
                    vec_F_global(1:size(K_tilde,1)) = vec_F;
                    vec_F_global(size(K_tilde,1)+1:end) = -kappa(aux)*vec_R;
                    %  figure;spy(K_global);title('structure K global');
        % MAJ du systeme sur K_global pour tenir compte des CL
        % par substitution
                    [Ks_global,Fs_global,U_impose_global,vec_n_DDL_conserves_global,vec_n_DDL_supprimes_global] = maj_matrices(K_global,vec_F_global,liste_DL_bloque);
        ...
        % test de l'admissibilite des valeurs identifiees par rapport aux bornes d'identification
                    if ( test_identification_lambda && test_identification_mu )
                        ...
                        if ( ~isempty(vec_n_param_min_lambda_reel) )
                            vec_lambda_noeuds_pha_identifies(vec_n_param_min_lambda_reel) = real(liste_proprietes_iterations{aux,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_lambda_reel)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*real((val_borne_min_lambda-liste_proprietes_iterations{aux,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_lambda_reel))))+1i*imag(vec_lambda_noeuds_pha_identifies(vec_n_param_min_lambda_reel)).';
                        end
                        if ( ~isempty(vec_n_param_max_lambda_reel) )
                            vec_lambda_noeuds_pha_identifies(vec_n_param_max_lambda_reel) = real(liste_proprietes_iterations{aux,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_lambda_reel)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*real((val_borne_max_lambda-liste_proprietes_iterations{aux,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_lambda_reel))))+1i*imag(vec_lambda_noeuds_pha_identifies(vec_n_param_max_lambda_reel)).';
                        end
                        if ( ~isempty(vec_n_param_min_lambda_imag) )
                            vec_lambda_noeuds_pha_identifies(vec_n_param_min_lambda_imag) = real(vec_lambda_noeuds_pha_identifies(vec_n_param_min_lambda_imag)).'+1i*imag(liste_proprietes_iterations{aux,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_lambda_imag)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*imag((val_borne_min_lambda-liste_proprietes_iterations{aux,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_lambda_imag))));
                        end
                        if ( ~isempty(vec_n_param_max_lambda_imag) )
                            vec_lambda_noeuds_pha_identifies(vec_n_param_max_lambda_imag) = real(vec_lambda_noeuds_pha_identifies(vec_n_param_max_lambda_imag)).'+1i*imag(liste_proprietes_iterations{aux,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_lambda_imag)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*imag((val_borne_max_lambda-liste_proprietes_iterations{aux,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_lambda_imag))));
                        end
                        ...
                        if ( ~isempty(vec_n_param_min_mu_reel) )
                            vec_mu_noeuds_pha_identifies(vec_n_param_min_mu_reel) = real(liste_proprietes_iterations{aux,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_mu_reel)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*real((val_borne_min_mu-liste_proprietes_iterations{aux,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_mu_reel))))+1i*imag(vec_mu_noeuds_pha_identifies(vec_n_param_min_mu_reel)).';
                        end
                        if ( ~isempty(vec_n_param_max_mu_reel) )
                            vec_mu_noeuds_pha_identifies(vec_n_param_max_mu_reel) = real(liste_proprietes_iterations{aux,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_mu_reel)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*real((val_borne_max_mu-liste_proprietes_iterations{aux,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_mu_reel))))+1i*imag(vec_mu_noeuds_pha_identifies(vec_n_param_max_mu_reel)).';
                        end
                        if ( ~isempty(vec_n_param_min_mu_imag) )
                            vec_mu_noeuds_pha_identifies(vec_n_param_min_mu_imag) = real(vec_mu_noeuds_pha_identifies(vec_n_param_min_mu_imag)).'+1i*imag(liste_proprietes_iterations{aux,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_mu_imag)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*imag((val_borne_min_mu-liste_proprietes_iterations{aux,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_mu_imag))));
                        end
                        if ( ~isempty(vec_n_param_max_mu_imag) )
                            vec_mu_noeuds_pha_identifies(vec_n_param_max_mu_imag) = real(vec_mu_noeuds_pha_identifies(vec_n_param_max_mu_imag)).'+1i*imag(liste_proprietes_iterations{aux,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_mu_imag)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*imag((val_borne_max_mu-liste_proprietes_iterations{aux,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_mu_imag))));
                        end
        ...
        % test de la convergence
            vec_difference_proprietes = liste_proprietes_iterations{aux,n_iter_LDC}-mat_proprietes_identifies_moyennes_sub_zones;
            if ( norm(vec_difference_proprietes) < tolerance_LDC*norm(liste_proprietes_iterations{aux,n_iter_LDC}) )
        ...
        %     title('maillage toutes sub-zones');
            ...
            disp(['        norme 1 valeurs identifies = ' num2str(sum(abs(mat_proprietes_identifies_moyennes_sub_zones),2)/size(mat_proprietes_identifies_moyennes_sub_zones,2)) ', norme relative de la correction = ' num2str(norm(vec_difference_proprietes)/norm(liste_proprietes_iterations{aux,n_iter_LDC}))]);
            disp(' ');
            
        % mise a jour des proprietes
            n_iter_LDC = n_iter_LDC+1;
            liste_proprietes_iterations{aux,n_iter_LDC} = mat_proprietes_identifies_moyennes_sub_zones;
        ...
        cd(path_dir{2+j_param});

        for nn_param = 1:size(liste_proprietes_iterations{aux,n_iter_LDC},1)
            ...
            % gcf = figure;
            % hold on;
			% plot(real(liste_proprietes_iterations{i_param,n_iter_LDC}(nn_param,:)),'-r');
            % plot(imag(liste_proprietes_iterations{i_param,n_iter_LDC}(nn_param,:)),'-b');
            % grid;
            ...
            plot(real(liste_proprietes_iterations{aux,n_iter_LDC}(nn_param,:)),'--r');
            plot(imag(liste_proprietes_iterations{aux,n_iter_LDC}(nn_param,:)),'--b');

			plot(real(liste_proprietes_iterations{aux,1}(nn_param,:))*ones(length(liste_proprietes_iterations{aux,n_iter_LDC}(nn_param,:))),'-k');
			plot(imag(liste_proprietes_iterations{aux,1}(nn_param,:))*ones(length(liste_proprietes_iterations{aux,n_iter_LDC}(nn_param,:))),'-k');

			title(sprintf('Material property $$\\mu$$ ($\\kappa$ = %d)', kappa(1)),'FontSize', 14, 'Interpreter', 'latex');
			xlabel('Number of iterations');
			ylabel('Values of $\\mu$ [Pa]','FontSize', 12, 'interpreter', 'latex');
			grid;
			legend({'Re $\left(\tilde{\mu}\right)$','Im $\left(\tilde{\mu}\right)$','Re $\left(\mu\right)$','Im $\left(\mu\right)$'},'interpreter','LaTeX');

			save(a,sprintf('results_kappa_%d.png',aux));

			a;

        end

        vec_param_identifie_moyen = zeros(size(liste_proprietes_iterations{aux,n_iter_LDC},1),n_iter_LDC_max);

        for n_iter_LDC = 1:n_iter_LDC_max

            for n_param = 1:size(liste_proprietes_iterations{aux,n_iter_LDC},1)

                vec_param_identifie_moyen(n_param,n_iter_LDC) = mean(liste_proprietes_iterations{aux,n_iter_LDC}(n_param,:));
    ...
    gcf = figure;
    plot([1:length(devFactor)], sTime./3600, '-k');
    grid;
    ylabel('Simulation time [h]');
    xlabel('Initial value index');
