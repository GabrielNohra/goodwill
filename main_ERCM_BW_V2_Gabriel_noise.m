...
while count <= sizeM
    ...
    flag = processText(path_dir{end}, amplitude_bruit_Gaussien_U, aux, kappa, muTheoretical);
    ...
    while ( (~test_convergence_LDC) && ( n_iter_LDC <= nb_iter_LDC_max) ) % Debut du critere sur la convergence (utile pour id)
        ...
        processText( path_dir{end}, n_iter_LDC, fcnError(n_iter_LDC,1), fcnError(n_iter_LDC,2), fcnError(n_iter_LDC+1,1), fcnError(n_iter_LDC+1,2), devMes, vec_U_mes );
        ...
        processText(path_dir{end}, flag, mat_proprietes_identifies_moyennes_sub_zones, liste_proprietes_iterations, vec_difference_proprietes);
