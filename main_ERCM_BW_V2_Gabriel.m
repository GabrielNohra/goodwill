...
nom_fichier_deplacement = '/users/bionanomri/nohra/Documents/MATLAB/data/donnees_dep_cisaillement';
...
while ( (~test_convergence_LDC) && ( n_iter_LDC <= nb_iter_LDC_max) && idx <= length(kappa) ) % Debut du critere sur la convergence (utile pour id)
...
for nn_param = 1:size(liste_proprietes_iterations{idx,n_iter_LDC},1)
    ...
    figure;
    hold on;
    plot(real(liste_proprietes_iterations{idx,n_iter_LDC}(nn_param,:)),'-r');
    plot(imag(liste_proprietes_iterations{idx,n_iter_LDC}(nn_param,:)),'-b');
    grid;
    xlabel('numero noeud phase');ylabel([nom_param ' (Pa)']);legend('reel','imag');
end

vec_param_identifie_moyen = zeros(size(liste_proprietes_iterations{idx,n_iter_LDC},1),n_iter_LDC_max);
