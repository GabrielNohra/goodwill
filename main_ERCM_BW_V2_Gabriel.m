...
for j_param = 1:2
    for i_param = 1:length(devFactor)
        ...
        sTime = zeros(1,i_param);
        nb_iter_LDC_max = 200;
        liste_proprietes_iterations = cell(i_param,nb_iter_LDC_max+1);
        ...
        liste_proprietes_iterations{i_param,n_iter_LDC} = struct_param_comportement_a_identifier.mat_param(struct_param_comportement_a_identifier.vec_numeros_parametres_a_identifier,:);
        ...
        % test de l'admissibilite des valeurs identifiees par rapport aux bornes d'identification
                    if ( test_identification_lambda && test_identification_mu )
                        ...
                        if ( ~isempty(vec_n_param_min_lambda_reel) )
                            vec_lambda_noeuds_pha_identifies(vec_n_param_min_lambda_reel) = real(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_lambda_reel)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*real((val_borne_min_lambda-liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_lambda_reel))))+1i*imag(vec_lambda_noeuds_pha_identifies(vec_n_param_min_lambda_reel)).';
                        end
                        if ( ~isempty(vec_n_param_max_lambda_reel) )
                            vec_lambda_noeuds_pha_identifies(vec_n_param_max_lambda_reel) = real(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_lambda_reel)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*real((val_borne_max_lambda-liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_lambda_reel))))+1i*imag(vec_lambda_noeuds_pha_identifies(vec_n_param_max_lambda_reel)).';
                        end
                        if ( ~isempty(vec_n_param_min_lambda_imag) )
                            vec_lambda_noeuds_pha_identifies(vec_n_param_min_lambda_imag) = real(vec_lambda_noeuds_pha_identifies(vec_n_param_min_lambda_imag)).'+1i*imag(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_lambda_imag)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*imag((val_borne_min_lambda-liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_lambda_imag))));
                        end
                        if ( ~isempty(vec_n_param_max_lambda_imag) )
                            vec_lambda_noeuds_pha_identifies(vec_n_param_max_lambda_imag) = real(vec_lambda_noeuds_pha_identifies(vec_n_param_max_lambda_imag)).'+1i*imag(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_lambda_imag)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*imag((val_borne_max_lambda-liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_lambda_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_lambda_imag))));
                        end
                        ...
                        if ( ~isempty(vec_n_param_min_mu_reel) )
                            vec_mu_noeuds_pha_identifies(vec_n_param_min_mu_reel) = real(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_mu_reel)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*real((val_borne_min_mu-liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_mu_reel))))+1i*imag(vec_mu_noeuds_pha_identifies(vec_n_param_min_mu_reel)).';
                        end
                        if ( ~isempty(vec_n_param_max_mu_reel) )
                            vec_mu_noeuds_pha_identifies(vec_n_param_max_mu_reel) = real(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_mu_reel)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*real((val_borne_max_mu-liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_mu_reel))))+1i*imag(vec_mu_noeuds_pha_identifies(vec_n_param_max_mu_reel)).';
                        end
                        if ( ~isempty(vec_n_param_min_mu_imag) )
                            vec_mu_noeuds_pha_identifies(vec_n_param_min_mu_imag) = real(vec_mu_noeuds_pha_identifies(vec_n_param_min_mu_imag)).'+1i*imag(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_mu_imag)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*imag((val_borne_min_mu-liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_min_mu_imag))));
                        end
                        if ( ~isempty(vec_n_param_max_mu_imag) )
                            vec_mu_noeuds_pha_identifies(vec_n_param_max_mu_imag) = real(vec_mu_noeuds_pha_identifies(vec_n_param_max_mu_imag)).'+1i*imag(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_mu_imag)))+struct_param_comportement_a_identifier.seuil_relaxation_bornes_min_max*imag((val_borne_max_mu-liste_proprietes_iterations{i_param,n_iter_LDC}(n_param_mu_ident,vec_n_noeuds_pha_sub_zone(vec_n_param_max_mu_imag))));
                        end
        ...
        % test de la convergence
            vec_difference_proprietes = liste_proprietes_iterations{i_param,n_iter_LDC}-mat_proprietes_identifies_moyennes_sub_zones;
            if ( norm(vec_difference_proprietes) < tolerance_LDC*norm(liste_proprietes_iterations{i_param,n_iter_LDC}) )
        ...
        %     title('maillage toutes sub-zones');
            ...
            disp(['        norme 1 valeurs identifies = ' num2str(sum(abs(mat_proprietes_identifies_moyennes_sub_zones),2)/size(mat_proprietes_identifies_moyennes_sub_zones,2)) ', norme relative de la correction = ' num2str(norm(vec_difference_proprietes)/norm(liste_proprietes_iterations{i_param,n_iter_LDC}))]);
            disp(' ');
            
        % mise a jour des proprietes
            n_iter_LDC = n_iter_LDC+1;
            liste_proprietes_iterations{i_param,n_iter_LDC} = mat_proprietes_identifies_moyennes_sub_zones;
        ...
        for nn_param = 1:size(liste_proprietes_iterations{i_param,n_iter_LDC},1)
        ...
        vec_param_identifie_moyen = zeros(size(liste_proprietes_iterations{i_param,n_iter_LDC},1),n_iter_LDC_max);

        for n_iter_LDC = 1:n_iter_LDC_max

            for n_param = 1:size(liste_proprietes_iterations{i_param,n_iter_LDC},1)

                vec_param_identifie_moyen(n_param,n_iter_LDC) = mean(liste_proprietes_iterations{i_param,n_iter_LDC}(n_param,:));
